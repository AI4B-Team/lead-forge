...
/** De-emphasized preview for the pre-agent state — nothing to answer from yet. */
  ...
  return (
    <div className="flex flex-wrap gap-2">
      {SAMPLE_QUESTIONS.map((q) => (
        <span
          key={q}
          className="rounded-full border border-dashed border-border bg-background px-3.5 py-1.5 text-xs font-medium text-muted-foreground"
        >
          “{q}”
        </span>
      ))}
