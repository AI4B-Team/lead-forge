...
import { canonicalRecordType, recordTypeId, RECORD_TYPE_OPTIONS, REQUEST_RECORD_TYPE, templateForRecordType } from "@/lib/record-types";
...
export function JobSpecCard({
  ...
  const countyVerified = (label: string) => {
    ...
    const typeKey = recordTypeId(recordTypeNow) ?? recordTypeNow;
    return verified.some(
      (r) =>
        r.status === "verified" &&
        (r.county_name ?? "").toLowerCase() === county.toLowerCase() &&
        (!state || r.state.toUpperCase() === state) &&
        (!typeKey || (recordTypeId(r.record_type) ?? r.record_type) === typeKey),
    );
  };
