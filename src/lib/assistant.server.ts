...
function systemPrompt(coveredCounties: string[], niches: string[], recordTypes: string[], templates: string): string {
  return [
    ...
    "- On the mismatch turn, do not patch sourceType or templateId. Patch geography and options you can already infer (state, counties, mobileOnly, etc.). Once the operator confirms, patch sourceType/templateId/recordType and continue.",
