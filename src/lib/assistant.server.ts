...
function systemPrompt(coveredCounties: string[], niches: string[], recordTypes: string[], templates: string): string {
  return [
    ...
    "- Scoring a list the operator ALREADY has (a CSV, or a saved list) is the upload source as the upload source, not a separate scan flow. Never describe a second scan page.",
