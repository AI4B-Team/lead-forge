...
export const Route = createFileRoute("/_authenticated/app/admin/workspaces")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Manage every workspace: plans, monthly caps, and credit grants." },
    ],
  }),
      ...
      <SettingsShell current="admin-workspaces">
        <AdminGate gate={gate}>
                      ...
                      <TableCell>
                        <div className="flex items-center gap-1">
                          ...
                          <Button
                            size="sm" variant="ghost" className="h-8 w-8 p-0"
                            title="Grant credits"
                            onClick={() => { setGrantWs(w); setGrantKind("sms"); setGrantAmount("10000"); }}
                          >
          ...
          <Dialog open={!!capWs} onOpenChange={(o) => !o && setCapWs(null)}>
            <DialogContent>
              <DialogHeader><DialogTitle>Monthly SMS Cap — {capWs?.name}</DialogTitle></DialogHeader>
              <div className="space-y-2 text-sm">
                <Label htmlFor="cap">Cap (leave blank for unlimited)</Label>
                <Input id="cap" inputMode="numeric" placeholder="e.g. 50000" value={capValue} onChange={(e) => setCapValue(e.target.value)} />
          ...
          <Dialog open={!!grantWs} onOpenChange={(o) => !o && setGrantWs(null)}>
            <DialogContent>
              <DialogHeader><DialogTitle>Grant Credits — {grantWs?.name}</DialogTitle></DialogHeader>
              <div className="space-y-3 text-sm">
                ...
                <div className="space-y-1">
                  ...
                  <Input id="amt" inputMode="numeric" value={grantAmount} onChange={(e) => setGrantAmount(e.target.value)} />
